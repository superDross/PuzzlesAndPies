...
This is an updated solution which is 300 times faster than my previous attempt.
'''
import numpy as np
...
def largest_product(arr):
    ...
    for i in range(20):
        col = arr[:, i]
        row = arr[i, :]
        diag = arr.diagonal(i) if i < 11 else arr.diagonal(-(i-10))
        flip_diag = np.flip(arr, axis=1).diagonal(i) if i < 11 else np.flip(arr, axis=1).diagonal(-(i-10))
        
        for d in [col, row, diag, flip_diag]:
            maximum = max_sum_fours(d)
            if maximum > product:
                product = maximum
    return product
